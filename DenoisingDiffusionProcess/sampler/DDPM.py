...
class DDPM_Sampler(nn.Module):
    ...
    def step(self, x_t, t, z_t):    
        ...
        # Approximate Distribution of Previos Sample in chain
        mean_pred, std_pred = self.posterior_params(x_t,t,z_t)
        # Sample from the distribution
        z= torch.randn_like(x_t) if any (t>0) else torch.randn_like(x_t)
    ...
    def posterior_params(self, x_t, t, noise_pred):
        ...
        alpha_one_minus_cumprod_sqrt_t = self.alphas_one_minus_cumprod_sqrt[t].view(b,1,1,1)
        alpha_sqrt_recip_t = self.alphas_sqrt_recip[t].view(b,1,1,1)
        
        mean = alpha_sqrt_recip_t * (x_t - beta_t * noise_pred/ alpha_one_minus_cumprod_sqrt_t)
        std = self.beta_sqrt[t].view(b,1,1,1) 
